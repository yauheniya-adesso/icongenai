"""
Step 7 — Batch-generate SVG icons from test-set prompts.

Reads data/test.jsonl (ChatML format from 05_prepare.py), generates one SVG
per prompt using the fine-tuned model + LoRA adapter, and writes results to
results/generated.jsonl for downstream evaluation by 08_evaluate.py.
...
Usage:
    ...
    uv run scripts/07_generate.py --no-adapter          # zero-shot baseline
    uv run scripts/07_generate.py --adapter models/icongenai-lora \\
